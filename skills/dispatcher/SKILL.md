...
每次失败后记录：失败类型、已采取的响应、当前重试次数。同时调用 `cache-writer`，传入 `outcome: failure`（首次失败）或 `outcome: retry`（重试中）、`failure_type`（当前分类）、`retry_count`（当前次数）、`signals_matched`（步骤 2 第一阶段）、`injections_fired: none`（失败路径不经过注入扫描）。
