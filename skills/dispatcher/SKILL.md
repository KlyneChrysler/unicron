...
调用 CTO 的两阶段组建流程：
1. **第一阶段（内容分析）**：读取任务标题、描述、验收标准和文件列表，对照 CTO 定义的信号表独立匹配所有触发信号。计划的 `Agents:` 字段是建议，不是约束。
2. **第二阶段（记忆调整）**：以 `phase`（当前阶段）和 `task`（当前任务标题和描述）为参数调用 `memory-reader`，将 `inform_dispatch` 条目应用于团队调整。
...
每次失败后记录：失败类型、已采取的响应、当前重试次数。

**重试上限：** 每个 Agent 每种失败类型最多重试 2 次。达到上限后：
```
