...
**实现：** [规格说明章节编号 + 需求编号，如"§3 需求 4"。使用 spec.md 中的实际章节编号；若规格说明未编号，按章节顺序标注 §1、§2…]
...
| Bug 修复 | backend-dev 或 frontend-dev（视受影响层而定）→ qa-engineer |
| 重构 | solutions-architect（验证目标架构）→ backend-dev 或 frontend-dev → code-reviewer |
| 测试套件 | qa-engineer |
...
示例违规：`- [ ] 创建用户模型，然后创建认证端点`（两个交付物）→ 拆分为任务 N.M（用户模型）和任务 N.M+1（认证端点，依赖于 N.M）。
